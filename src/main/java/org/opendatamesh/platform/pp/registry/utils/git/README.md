...
The git package (`org.opendatamesh.platform.pp.registry.utils.git`) is organized into:

- **`provider/`**: Core provider interfaces and implementations (GitHub, GitLab, Bitbucket, Azure DevOps), factory, and provider-specific credentials
...
The registry’s `ResponseExceptionHandler` maps these to HTTP responses:
...
Services may catch `GitOperationException` (or other git exceptions) and rethrow as `BadRequestException` or other `RegistryApiException` to add context; if they do not, the handler above still returns a proper response instead of a generic 500. `GitProviderConfigurationException` is handled by the registry and returned as 400 Bad Request.
...
```java
package org.opendatamesh.platform.pp.registry.git.provider.yourprovider.modelextensions;
...
```java
package org.opendatamesh.platform.pp.registry.git.provider.yourprovider;
...
- **Exception Hierarchy**: `GitException` base with `GitOperationException`, `GitProviderAuthenticationException`, and `GitClientException`, mapped to consistent HTTP responses by the REST layer
