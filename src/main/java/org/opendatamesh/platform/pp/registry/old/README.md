...
|--------|--------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `GET`  | `/api/v1/pp/registry/products`                                           | Returns a list of Data Products matching the given FQN (v1.x.x format)                               | **Query:**<br/>- `fqn`: Fully qualified name filter (exact match, case-insensitive). If omitted, returns an empty list.                                                                   | List of `RegistryV1DataProductResource` with: `id`, `fullyQualifiedName`, `description`, `domain`                                                                            | • **Legacy Client Integration**: List/find products by FQN without migrating to newer API<br/>• **Search by FQN**: Uses the same search pattern as core (DataProductSearchOptions by FQN)                        |
| `GET`  | `/api/v1/pp/registry/products/{uuid}`                                    | Retrieves a Data Product by identifier in the v1.x.x format                                           | **Path:**<br/>- `uuid`: Data Product identifier (supports both FQN-derived ID from `IdentifierStrategy.getId(fqn)` and real V2 data product UUID)                                                                                                        | `RegistryV1DataProductResource` with: `id`, `fullyQualifiedName`, `description`, `domain`                                                                                    | • **Legacy Client Integration**: Get product metadata without migrating to newer API<br/>• **Dual ID Support**: Use either FQN-derived ID (legacy) or database UUID (V2)                                        |
...
| `POST` | `/api/v1/pp/registry/validate/report`                                    | Returns a V1 validation report for a Data Product Version descriptor (syntax and/or policy validation) | **Body:** `RegistryV1DataProductValidationRequestResource`<br/>- `validateSyntax`: run structural validation (default `true`)<br/>- `validatePolicies`: run policy validation via Policy Service V1 (default `true`)<br/>- `dataProductVersion`: DPDS descriptor (JSON) | `RegistryV1DataProductValidationResponseResource`: `syntaxValidationResult` (when syntax requested), `policiesValidationResults` (map of policy name → result). Always 200 OK; validity is in the body. On policy client exception returns 500 with `ServerError`. | • **Syntax-Only**: Request `validateSyntax=true`, `validatePolicies=false` — get descriptor structure outcome; `policiesValidationResults` empty<br/>• **Policy-Only**: Request `validatePolicies=true`, `validateSyntax=false` — get policy results per evaluated policy<br/>• **Both**: Request both flags — syntax + policy results; policy client invoked for data product and data product version policies<br/>• **Syntax Failures**: Invalid descriptor (e.g. missing `info`) → `syntaxValidationResult.validated=false`, `validationOutput` with error details<br/>• **Policy Failures**: Policy service returns violation → corresponding entry in `policiesValidationResults` with `validated=false`<br/>• **Policy Disabled**: If Policy Service is not active and `validatePolicies=true`, response includes `PolicyServiceNotActive` in `policiesValidationResults` with explanatory message<br/>• **Policy State**: When validating policies, currentState = most recent existing DPV for same FQN; afterState = passed descriptor; both parsed with old parser and sent in event format to Policy Service V1 |
...
- The Get Variables endpoint automatically creates missing variables referenced in descriptors but not yet stored in the
  database
- **Validate Report**: The POST validate/report endpoint returns HTTP 200 for all validation outcomes; validity is
  expressed in the response body (`syntaxValidationResult.validated`, per-policy `validated` in `policiesValidationResults`).
  When policy validation is requested, the most recent existing Data Product Version for the same FQN is used as
  `currentState` (beforeState) and the request descriptor as `afterState`; both are parsed with the old parser and sent
  to the Policy Service V1 in event format. If the policy client throws an unexpected exception, the endpoint returns
  500 with error type `ServerError`.
